...
/*---------------------------- SPIx CRCPOLY Configuration --------------------*/
  ...
  SPI_CRCPR(SPIx) = SPI_InitStruct->SPI_CRCPolynomial;
}
 ...
 if (NewState != DISABLE)
  {
    ...
    SPI_CR1(SPIx) |= CR1_SPE_Set;
  }
  else
  {
    /* Disable the selected SPI peripheral */
    SPI_CR1(SPIx) &= CR1_SPE_Reset;
  }
}
 ...
 if (NewState != DISABLE)
  {
    ...
    SPI_CR2(SPIx) |= SPI_I2S_DMAReq;
  }
  else
  {
    /* Disable the selected SPI/I2S DMA requests */
    SPI_CR2(SPIx) &= (uint16_t)~SPI_I2S_DMAReq;
  }
}
...
void SPI_NSSInternalSoftwareConfig(uint32_t SPIx, uint16_t SPI_NSSInternalSoft)
{
  if (SPI_NSSInternalSoft != SPI_NSSInternalSoft_Reset)
  {
    ...
    SPI_CR1(SPIx) |= SPI_NSSInternalSoft_Set;
  }
  else
  {
    /* Reset NSS pin internally by software */
    SPI_CR1(SPIx) &= SPI_NSSInternalSoft_Reset;
  }
}
...
void SPI_SSOutputCmd(uint32_t SPIx, FunctionalState NewState)
{
	if (NewState != DISABLE)
  {
    ...
    SPI_CR2(SPIx) |= CR2_SSOE_Set;
  }
  else
  {
    /* Disable the selected SPI SS output */
    SPI_CR2(SPIx) &= CR2_SSOE_Reset;
  }
}
...
{
	/* Clear DFF bit */
  SPI_CR1(SPIx) &= (uint16_t)~SPI_DataSize_16b;
  /* Set new DFF bit value */
  SPI_CR1(SPIx) |= SPI_DataSize;
}
...
void SPI_CalculateCRC(uint32_t SPIx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    SPI_CR1(SPIx) |= CR1_CRCEN_Set;
  }
  else
  {
    /* Disable the selected SPI CRC calculation */
    SPI_CR1(SPIx) &= CR1_CRCEN_Reset;
  }
}
...
uint16_t SPI_GetCRC(uint32_t SPIx, uint8_t SPI_CRC)
{
  ...
  if (SPI_CRC != SPI_CRC_Rx)
  {
    ...
    crcreg = SPI_TXCRCR(SPIx);
  }
  else
  {
    /* Get the Rx CRC register */
    crcreg = SPI_RXCRCR(SPIx);
  }
 ...
 if (SPI_Direction == SPI_Direction_Tx)
  {
    ...
    SPI_CR1(SPIx) |= SPI_Direction_Tx;
  }
  else
  {
    /* Set the Rx only mode */
    SPI_CR1(SPIx) &= SPI_Direction_Rx;
  }
}
...
FlagStatus SPI_I2S_GetFlagStatus(uint32_t SPIx, uint16_t SPI_I2S_FLAG)
{
  ...
  if ((SPI_SR(SPIx) & SPI_I2S_FLAG) != (uint16_t)RESET)
  {
    /* SPI_I2S_FLAG is set */
    bitstatus = SET;
  }
...
{  
    ...
    SPI_SR(SPIx) = (uint16_t)~SPI_I2S_FLAG;
}
...
ITStatus SPI_I2S_GetITStatus(uint32_t SPIx, uint8_t SPI_I2S_IT)
{
  ...
  enablestatus = (SPI_CR2(SPIx) & itmask) ;

  /* Check the status of the specified SPI/I2S interrupt */
  if (((SPI_SR(SPIx) & itpos) != (uint16_t)RESET) && enablestatus)
  {
    /* SPI_I2S_IT is set */
    bitstatus = SET;
  }
