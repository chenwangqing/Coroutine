...
/*---------------------------- USART CR2 Configuration -----------------------*/
  ...
  /* Write to USART CR2 */
  USART_CR2(USARTx) = (uint16_t)tmpreg;

/*---------------------------- USART CR1 Configuration -----------------------*/
  tmpreg = USART_CR1(USARTx);
  ...
  /* Write to USART CR1 */
  USART_CR1(USARTx) = (uint16_t)tmpreg;

/*---------------------------- USART CR3 Configuration -----------------------*/  
  tmpreg = USART_CR3(USARTx);
  ...
  /* Write to USART CR3 */
  USART_CR3(USARTx) = (uint16_t)tmpreg;
...
/*---------------------------- USART BRR Configuration -----------------------*/
  ...
  if ((USART_CR1(USARTx) & CR1_OVER8_Set) != 0)
  {
    /* Integer part computing in case Oversampling mode is 8 Samples */
    integerdivider = ((25 * apbclock) / (2 * (USART_InitStruct->USART_BaudRate)));    
  }
  else /* if ((USART_CR1(USARTx) & CR1_OVER8_Set) == 0) */
  {
  ...
  /* Write to USART BRR */
  USART_BRR(USARTx) = (uint16_t)tmpreg;
}
...
{ 
  if (NewState != DISABLE)
  {
    ...
    USART_CR1(USARTx) |= CR1_UE_Set;
  }
  else
  {
    /* Disable the selected USART by clearing the UE bit in the CR1 register */
    USART_CR1(USARTx) &= CR1_UE_Reset;
  }
}
...
/**
  ...
  *   This parameter can be: ENABLE or DISABLE.
  * @note The DMA mode is not available for UART5 except in the HC32
  *       High density value line devices(hc32f10x_HD_VL).  
  * @retval None
  */
void USART_DMACmd(uint32_t USARTx, uint16_t USART_DMAReq, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR3(USARTx) |= USART_DMAReq;
  }
  else
  {
    /* Disable the DMA transfer for selected requests by clearing the DMAT and/or
       DMAR bits in the USART CR3 register */
    USART_CR3(USARTx) &= (uint16_t)~USART_DMAReq;
  }
}
...
{ 
  ...
  USART_CR2(USARTx) &= CR2_Address_Mask;
  /* Set the USART address node */
  USART_CR2(USARTx) |= USART_Address;
}
...
{
  USART_CR1(USARTx) &= CR1_WAKE_Mask;
  USART_CR1(USARTx) |= USART_WakeUp;
}
...
void USART_ReceiverWakeUpCmd(uint32_t USARTx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR1(USARTx) |= CR1_RWU_Set;
  }
  else
  {
    /* Disable the USART mute mode by clearing the RWU bit in the CR1 register */
    USART_CR1(USARTx) &= CR1_RWU_Reset;
  }
}
...
{
  USART_CR2(USARTx) &= CR2_LBDL_Mask;
  USART_CR2(USARTx) |= USART_LINBreakDetectLength;  
}
...
void USART_LINCmd(uint32_t USARTx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR2(USARTx) |= CR2_LINEN_Set;
  }
  else
  {
    /* Disable the LIN mode by clearing the LINEN bit in the CR2 register */
    USART_CR2(USARTx) &= CR2_LINEN_Reset;
  }
}
...
{
  /* Transmit Data */
  USART_DR(USARTx) = (Data & (uint16_t)0x01FF);
}
...
{
  /* Receive Data */
  return (uint16_t)(USART_DR(USARTx) & (uint16_t)0x01FF);
}
...
{    
  ...
  USART_GTPR(USARTx) &= GTPR_LSB_Mask;
  /* Set the USART guard time */
  USART_GTPR(USARTx) |= (uint16_t)((uint16_t)USART_GuardTime << 0x08);
}
...
{ 
  ...
  USART_GTPR(USARTx) &= GTPR_MSB_Mask;
  /* Set the USART prescaler */
  USART_GTPR(USARTx) |= USART_Prescaler;
}
 ...
 if (NewState != DISABLE)
  {
    ...
    USART_CR3(USARTx) |= CR3_SCEN_Set;
  }
  else
  {
    /* Disable the SC mode by clearing the SCEN bit in the CR3 register */
    USART_CR3(USARTx) &= CR3_SCEN_Reset;
  }
}
 ...
 if (NewState != DISABLE)
  {
    ...
    USART_CR3(USARTx) |= CR3_NACK_Set;
  }
  else
  {
    /* Disable the NACK transmission by clearing the NACK bit in the CR3 register */
    USART_CR3(USARTx) &= CR3_NACK_Reset;
  }
}
...
void USART_HalfDuplexCmd(uint32_t USARTx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR3(USARTx) |= CR3_HDSEL_Set;
  }
  else
  {
    /* Disable the Half-Duplex mode by clearing the HDSEL bit in the CR3 register */
    USART_CR3(USARTx) &= CR3_HDSEL_Reset;
  }
}
...
void USART_OverSampling8Cmd(uint32_t USARTx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR1(USARTx) |= CR1_OVER8_Set;
  }
  else
  {
    /* Disable the 8x Oversampling mode by clearing the OVER8 bit in the CR1 register */
    USART_CR1(USARTx) &= CR1_OVER8_Reset;
  }
}
...
void USART_OneBitMethodCmd(uint32_t USARTx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR3(USARTx) |= CR3_ONEBITE_Set;
  }
  else
  {
    /* Disable tthe one bit method by clearing the ONEBITE bit in the CR3 register */
    USART_CR3(USARTx) &= CR3_ONEBITE_Reset;
  }
}
...
{ 
  USART_CR3(USARTx) &= CR3_IRLP_Mask;
  USART_CR3(USARTx) |= USART_IrDAMode;
}
...
void USART_IrDACmd(uint32_t USARTx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    USART_CR3(USARTx) |= CR3_IREN_Set;
  }
  else
  {
    /* Disable the IrDA mode by clearing the IREN bit in the CR3 register */
    USART_CR3(USARTx) &= CR3_IREN_Reset;
  }
}
...
FlagStatus USART_GetFlagStatus(uint32_t USARTx, uint16_t USART_FLAG)
{
  ...
  if ((USART_SR(USARTx) & USART_FLAG) != (uint16_t)RESET)
  {
    bitstatus = SET;
  }
  else
...
{
  USART_SR(USARTx) = (uint16_t)~USART_FLAG;
}
...
ITStatus USART_GetITStatus(uint32_t USARTx, uint16_t USART_IT)
{
  ...
  if (usartreg == 0x01) /* The IT  is in CR1 register */
  {
    itmask &= USART_CR1(USARTx);
  }
  else if (usartreg == 0x02) /* The IT  is in CR2 register */
  {
    itmask &= USART_CR2(USARTx);
  }
  else /* The IT  is in CR3 register */
  {
    itmask &= USART_CR3(USARTx);
  }
  
  bitpos = USART_IT >> 0x08;
  bitpos = (uint32_t)0x01 << bitpos;
  bitpos &= USART_SR(USARTx);
...
void USART_ClearITPendingBit(uint32_t USARTx, uint16_t USART_IT)
{
  ...
  USART_SR(USARTx) = (uint16_t)~itmask;
}
