...
void ADC_Init(uint32_t ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  /* Write to ADCx CR1 */
  ADC_CR1(ADCx) = tmpreg1;

  /*---------------------------- ADCx CR2 Configuration -----------------*/
  /* Get the ADCx CR2 value */
  tmpreg1 = ADC_CR2(ADCx);
  ...
  /* Write to ADCx CR2 */
  ADC_CR2(ADCx) = tmpreg1;

  /*---------------------------- ADCx SQR1 Configuration -----------------*/
  /* Get the ADCx SQR1 value */
  tmpreg1 = ADC_SQR1(ADCx);
  /* Clear L bits */
...
void ADC_Cmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR2(ADCx) |= CR2_ADON_Set;
  }
  else
  {
    /* Disable the selected ADC peripheral */
    ADC_CR2(ADCx) &= CR2_ADON_Reset;
  }
}
...
void ADC_DMACmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR2(ADCx) |= CR2_DMA_Set;
  }
  else
  {
    /* Disable the selected ADC DMA request */
    ADC_CR2(ADCx) &= CR2_DMA_Reset;
  }
}
...
void ADC_SoftwareStartConvCmd(uint32_t ADCx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    /* Enable the selected ADC conversion on external event and start the selected
       ADC conversion */
    ADC_CR2(ADCx) |= CR2_EXTTRIG_SWSTART_Set;
  }
  else
  {
    /* Disable the selected ADC conversion on external event and stop the selected
       ADC conversion */
    ADC_CR2(ADCx) &= CR2_EXTTRIG_SWSTART_Reset;
  }
}
...
FlagStatus ADC_GetSoftwareStartConvStatus(uint32_t ADCx)
{
  ...
  if ((ADC_CR2(ADCx) & CR2_SWSTART_Set) != (uint32_t)RESET)
  {
    /* SWSTART bit is set */
    bitstatus = SET;
  }
...
void ADC_DiscModeCmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR1(ADCx) |= CR1_DISCEN_Set;
  }
  else
  {
    /* Disable the selected ADC regular discontinuous mode */
    ADC_CR1(ADCx) &= CR1_DISCEN_Reset;
  }
}
...
void ADC_RegularChannelConfig(uint32_t ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  if (ADC_Channel > ADC_Channel_9)
  {
    ...
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADC_SMPR1(ADCx) = tmpreg1;
  }
  else /* ADC_Channel include in ADC_Channel_[0..9] */
  {
    /* Get the old register value */
    tmpreg1 = ADC_SMPR2(ADCx);
    /* Calculate the mask to clear */
    ...
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADC_SMPR2(ADCx) = tmpreg1;
  }
  /* For Rank 1 to 6 */
  if (Rank < 7)
  {
    /* Get the old register value */
    tmpreg1 = ADC_SQR3(ADCx);
    /* Calculate the mask to clear */
    ...
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADC_SQR3(ADCx) = tmpreg1;
  }
  /* For Rank 7 to 12 */
  else if (Rank < 13)
  {
    /* Get the old register value */
    tmpreg1 = ADC_SQR2(ADCx);
    /* Calculate the mask to clear */
...
void ADC_ExternalTrigConvCmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR2(ADCx) |= CR2_EXTTRIG_Set;
  }
  else
  {
    /* Disable the selected ADC conversion on external event */
    ADC_CR2(ADCx) &= CR2_EXTTRIG_Reset;
  }
}
...
void ADC_AutoInjectedConvCmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR1(ADCx) |= CR1_JAUTO_Set;
  }
  else
  {
    /* Disable the selected ADC automatic injected group conversion */
    ADC_CR1(ADCx) &= CR1_JAUTO_Reset;
  }
}
...
void ADC_InjectedDiscModeCmd(uint32_t ADCx, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ADC_CR1(ADCx) |= CR1_JDISCEN_Set;
  }
  else
  {
    /* Disable the selected ADC injected discontinuous mode */
    ADC_CR1(ADCx) &= CR1_JDISCEN_Reset;
  }
}
...
void ADC_ExternalTrigInjectedConvCmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR2(ADCx) |= CR2_JEXTTRIG_Set;
  }
  else
  {
    /* Disable the selected ADC external event selection for injected group */
    ADC_CR2(ADCx) &= CR2_JEXTTRIG_Reset;
  }
}
...
void ADC_SoftwareStartInjectedConvCmd(uint32_t ADCx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR2(ADCx) |= CR2_JEXTTRIG_JSWSTART_Set;
  }
  else
  {
    /* Disable the selected ADC conversion on external event for injected group and stop the selected
       ADC injected conversion */
    ADC_CR2(ADCx) &= CR2_JEXTTRIG_JSWSTART_Reset;
  }
}
...
FlagStatus ADC_GetSoftwareStartInjectedConvCmdStatus(uint32_t ADCx)
{
  ...
  if ((ADC_CR2(ADCx) & CR2_JSWSTART_Set) != (uint32_t)RESET)
  {
    /* JSWSTART bit is set */
    bitstatus = SET;
  }
...
void ADC_AnalogWatchdogThresholdsConfig(uint32_t ADCx, uint16_t HighThreshold,uint16_t LowThreshold)
{
  ...
  ADC_HTR(ADCx) = HighThreshold;
  /* Set the ADCx low threshold */
  ADC_LTR(ADCx) = LowThreshold;
}
...
void ADC_TempSensorVrefintCmd(FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    ...
    ADC_CR2(ADC1) |= CR2_TSVREFE_Set;
  }
  else
  {
    /* Disable the temperature sensor and Vrefint channel*/
    ADC_CR2(ADC1) &= CR2_TSVREFE_Reset;
  }
}
...
FlagStatus ADC_GetFlagStatus(uint32_t ADCx, uint8_t ADC_FLAG)
{
  ...
  if ((ADC_SR(ADCx) & ADC_FLAG) != (uint8_t)RESET)
  {
    /* ADC_FLAG is set */
    bitstatus = SET;
  }
...
ITStatus ADC_GetITStatus(uint32_t ADCx, uint16_t ADC_IT)
{
  ...
  enablestatus = (ADC_CR1(ADCx) & (uint8_t)ADC_IT) ;
  /* Check the status of the specified ADC interrupt */
  if (((ADC_SR(ADCx) & itmask) != (uint32_t)RESET) && enablestatus)
  {
    /* ADC_IT is set */
    bitstatus = SET;
  }
